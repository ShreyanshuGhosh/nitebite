...
const ProductCard: React.FC<ProductCardProps> = ({ product, index }) => {
  ...
  const handleAddToCart = () => {
    // Use the override image (if provided) otherwise fall back to image_url.
    const imageForCart = product.image || product.image_url;
    // Remove both image_url and image from the product when adding it to the cart.
    const { image_url, image, ...rest } = product;
  ...
  return (
    ...
    >
      <div className="relative aspect-square overflow-hidden">
        <img 
          src={typeof product.image_url === 'string' ? product.image_url : (product.image as string)}
          alt={product.name}
