...
const OrderDetails = () => {
  ...
  // Expects couponDiscount to be a secure, read-only value from your store.
  ...
  // Check authentication and pre-fill contact details if available
  useEffect(() => {
    if (items.length === 0) {
      navigate('/checkout');
      return;
  ...
  // Calculate fees and totals using rupee currency
  const subtotal = calculateSubtotal();
  const validCouponDiscount = couponDiscount ? Math.min(couponDiscount, subtotal) : 0;
  const deliveryFee = subtotal > 0 ? 2.99 : 0;
  const serviceFee = subtotal > 0 ? subtotal * 0.05 : 0;
  const total = subtotal + deliveryFee + serviceFee - validCouponDiscount;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Build the orderData object as per your schema.
      const orderData = {
        user_id: user.id,
        amount: total,
      ...
      if (validCouponDiscount > 0) {
        // In a production app, the applied coupon code should be stored in your global state.
        // Here, for demonstration purposes, we're using "STUDENT10".
          ...
          <div className="glassmorphic-card p-4 md:p-6 rounded-2xl mb-8">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between text-nitebite-text">
                ...
                <span className="text-nitebite-highlight">₹{deliveryFee.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-nitebite-text">
                <span>Service Fee</span>
                <span className="text-nitebite-highlight">₹{serviceFee.toFixed(2)}</span>
              </div>
              {validCouponDiscount > 0 && (
                <div className="flex justify-between text-nitebite-warning">
                  <span>Coupon Discount</span>
                  <span>-₹{validCouponDiscount.toFixed(2)}</span>
                </div>
              )}
